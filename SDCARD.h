...
#define SD_CS PORTDbits.RD0
#define SD_CS_DIR TRISDbits.TRISD0
#define SD_CD PORTCbits.RC14
#define SD_CD_DIR TRISCbits.TRISC14
#define SDI PORTFits.RF4
#define SDI_DIR TRISFbits.TRISF4
#define SCK PORTBbits.RB14
#define SCK_DIR TRISBbits.TRISB14
#define SDO PORTFbits.RF5
#define SDO_DIR TRISFbits.TRISF5
...
buffer_typ buffer1[SD_BLOCK_SIZE * 8];
...
unsigned char setup_SDSPI(void) {
    unsigned char res = 0;
    unsigned long start_block = 0;//0xD6D6;
...
void SPIWrite(unsigned char data) {
    ...
    while (SPI4STATbits.SPIBUSY); // Initially it was SPITBF but cannot resolve
}
...
unsigned char SPIRead(void) {
    ...
    SPI4BUF = 0xFF; // write dummy data to initiate an SPI read
    while (SPI4STATbits.SPIBUSY); // wait until the data is finished reading
    data = SPI4BUF;
...
void addByteToBuffer(char characterToWrite, int bufferNumber) {
    if(bufferNumber == 0) {
