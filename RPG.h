...
void initializeRPG() {
    /*CNENbits.CNEN8 = 1; //enable CN8
    CNENbits.CNEN9 = 1;
    CNENbits.CNEN10 = 1; */
    CNPUEbits.CNPUE8 = 1; //Pullup for CN8
    CNPUEbits.CNPUE9 = 1;
    CNPUEbits.CNPUE10 = 1;
    /*
     ...
     */
    PORTSetPinsDigitalIn(IOPORT_G, BIT_6); //LSB
    PORTSetPinsDigitalIn(IOPORT_G, BIT_7); //MSB
    PORTSetPinsDigitalIn(IOPORT_G, BIT_8); //Push button
...
void pushbuttonloop()
{
    if(PORTGbits.RG8 != prevpb){
        if(PORTGbits.RG8 == 1)
            {prevpb = 1;}
        else if(PORTGbits.RG8 == 0){
...
void updateEncoder()
{
    MSB = PORTGbits.RG7 ; //MSB = most significant bit
    LSB = PORTGbits.RG6 ; //LSB = least significant bit
